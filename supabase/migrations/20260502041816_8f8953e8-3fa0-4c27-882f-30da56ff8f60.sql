CREATE OR REPLACE FUNCTION public.finalize_conversation(_conversation_id uuid, _outcome text, _reason text DEFAULT NULL::text, _value_cents integer DEFAULT NULL::integer, _stage_id uuid DEFAULT NULL::uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Busca deal ativo mais recente entre TODOS os contatos com este telefone (resolve duplicatas de contato)
  SELECT d.* INTO deal_rec
    FROM public.crm_deals d
    JOIN public.contacts ct ON ct.id = d.contact_id
   WHERE d.account_id = conv.account_id
     AND ct.account_id = conv.account_id
     AND ct.phone = conv.phone
